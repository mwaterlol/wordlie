...
import { assertExistence, enumerateLangs, I18nResolvedVirtualModuleId, I18nVirtualModuleId, jsNormalizedLang, loadAndParse, resolvePaths, } from './utils.js';
const factory = ({ paths, include = ['**/*.json', '**/*.yml', '**/*.yaml'], i18nNS = 'translation', debug, }) => {
    ...
    const plugin = {
        ...
        async handleHotUpdate({ file, server }) {
            const isLocaleFile = file.match(/\.(json|yml|yaml)$/) && paths.some((p) => file.startsWith(path.join(process.cwd(), p)));
            if (isLocaleFile) {
